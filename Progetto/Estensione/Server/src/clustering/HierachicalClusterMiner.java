...
public class HierachicalClusterMiner implements Serializable {
	 ...
	 * @throws InvalidDepthException se la profondità è minore o uguale a 0
	 */
	 ...
	 * @throws InvalidSizeException se si verifica un errore di dimensione
	 * @throws InvalidClustersNumberException se si verifica un errore nel numero di cluster
	 */
	...
	public void salva(String fileName) throws FileNotFoundException, IOException {
		// Sostituisce i separatori di percorso non validi con quelli di sistema
		fileName = fileName.replace("\\", File.separator).replace("/", File.separator);
	
		// Verifica la presenza di caratteri non validi nel percorso del file
		if (fileName.matches(".*[<>:\"|?*].*"))
			throw new IOException("Errore: Il percorso contiene caratteri non validi. Riprova.\n");
	
		File file = new File(fileName);
	
		// Se il file esiste, tentare di eliminarlo per evitare conflitti
		if (file.exists()) {
			if (!file.delete()) {
				throw new IOException("Errore: Il file esiste già e non può essere sovrascritto. Riprova.\n");
			}
		}
	
		// Controlla se la directory principale esiste, altrimenti prova a crearla
		...
		// Scrive l'oggetto nel file specificato
